...
public class BDGraphExtension<NodeType,EdgeType extends Number> implements GraphADT<NodeType, EdgeType>, IGraphExtension<NodeType> {
    ...
    public BDGraphExtension() { vertices = new Hashtable<>(); }
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        // check if both start vertex and end vertex can be found
        if (!(vertices.containsKey(start) && vertices.containsKey(end)))
            throw new NoSuchElementException("No start or end vertex");
        // use a priority queue to store the discovered paths
        PriorityQueue<Path> discoveredPaths = new PriorityQueue<Path>();
        // use a hashtable to store all the visited vertices
        Hashtable<NodeType, Vertex> visited = new Hashtable<NodeType, Vertex>();

        // initialization
        Path p = new Path(this.vertices.get(start));
        discoveredPaths.add(p);

        while (!(discoveredPaths.isEmpty())) {
            p = discoveredPaths.poll();
            visited.put(p.end.data, new Vertex(p.end.data));
            // if the end of the path is the end vertex, return the path
            if (p.end.data.equals(end)) return p;
            // add paths to unvisited vertices
            for (Edge edge : p.end.edgesLeaving) {
                if (!(visited.containsKey(edge.target.data)))
                    discoveredPaths.add(new Path(p, edge));
            }
        }
